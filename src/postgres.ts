...
export class PostgresWriter {
  ...
  writeBaseUniswapV4(pool: UniswapV4PoolRecord): void { this.enqueue('base_uniswap_v4_pools', this.camelToSnake(pool)); }
  ...
  writeBaseUniswapV4Price(price: import('./evm/bsc/uniswap-v4-price.js').UniswapV4Price): void { this.enqueue('base_uniswap_v4_prices', { pool_id: price.poolId, pool_address: price.poolAddress, price: price.price, inverse_price: price.inversePrice, base_currency: price.baseCurrency, quote_currency: price.quoteCurrency, sqrt_price_x96: price.sqrtPriceX96.toString(), liquidity: price.liquidity.toString(), tick: price.tick, fee: price.fee, updated_block: price.updatedBlock, updated_at: price.updatedAt }, 'pool_id'); }
